...
/* acessa a API e traz o ARRAY de pokemons que está em 'results' */
...
/* guarda um array de pokemons já detalhados por meio do acesso a url 
de cada pokemon que está dentro de 'results 

    - cria array para guardar pokemons
    - percorre cada pokemon recebido do fetchPokemons
    - usa a função fetchPokemon para acessar o url de cada pokemon
    - salva os dados no novo array
    obs: for of aceita funções assíncronas
*/
...
/* traz os dados de um pokemon acessando sua url*/
...
async function populatePokemons(pokemons){
    ...
    for (const PokemonData of allPokemonsData) {
        insertPokemonHTML(PokemonData)
    }
  }


/* insere cada pokemon num card da li */
...
/* inicia um addEventListener no input de busca pelo nome */
...
async function handleSearchInput(event, pokemons){
  ...
  const value = event.target.value;/* captura o valor digitado */
...
/* remove tds os pokemons da tela para ser renderizado apenas o buscado pelo usuario */
...
/* busca pelos filtros de types de pokemon 
    - captura o valor do atributo data-pokemon-type-name no html
    - popula um array (pokemonDataFilteredByType) com os pokemons do type selecionado
    - renderiza na tela os cards de acordo com o type
*/
...
/* inicia função de filtro nos buttons 
    - captura tds os filter-button
    - add evento de click e busca qual button foi clicado por meio do filterClicked
*/
